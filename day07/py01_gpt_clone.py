...
from tkinter.messagebox import * # 모듈 및에 있는 모듈을 from tkinter import * 로 가져올 수 있음
...
def responseMessage():
    ...
    print(inputText)
    textMessage.delete('1.0', END) # 1.0이 뭐노   입력창에 글 지우기 
    ...
    if inputText:
        try:
            ...
            textResult.insert(END, f'Eror: {str(e)}\m\n','error')
        finally:
...
# 12.아이콘 변경
# ./image/ 경로는 삭제
root.iconbitmap('./image/chatbot.ico') # pyinstaller때는 실행파일폴더 복사 

# 7. 전체에서 사용할 폰트 지정 -> 나눔고딕
myFont = Font(family='NanumGothic',size=10)
bolodFont = Font(family='NanumGothic',size=10, weight=BOLD, slant=ITALIC)
...
# 8. 입력창에서 엔터를 치면 keypress이벤트 처리함수 발생
textMessage.bind('<Key>',keypress)
textMessage.pack(side=LEFT, padx=15)

buttonSend = Button(inputFrame, text='전송', bg='green', fg='white',command=responseMessage,font=myFont)
...
# 10. 스크롤 덱스트에 나올 메시지 디자인
textResult.tag_configure('user', font=bolodFont, foreground = 'yellow')
textResult.tag_configure('ai', font=myFont, foreground='limegreen') # 89F336
textResult.tag_configure('error',font=bolodFont, foreground='red')
